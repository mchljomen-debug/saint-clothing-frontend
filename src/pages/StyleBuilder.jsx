...
const FIT_PRESETS={
  default:{
    top:{x:0,y:4,scale:0.92,width:315,height:250},
    bottom:{x:0,y:0,scale:0.88,width:300,height:260}
  },
  tshirt:{top:{x:0,y:0,scale:0.88,width:320,height:250}},
  "t-shirt":{top:{x:0,y:0,scale:0.88,width:320,height:250}},
  tee:{top:{x:0,y:0,scale:0.88,width:320,height:250}},
  "long sleeve":{top:{x:0,y:2,scale:0.9,width:325,height:255}},
  "crop jersey":{top:{x:0,y:12,scale:0.86,width:315,height:220}},
  jersey:{top:{x:0,y:8,scale:0.88,width:320,height:235}},
  jorts:{bottom:{x:0,y:-2,scale:0.86,width:300,height:245}},
  "mesh short":{bottom:{x:0,y:-4,scale:0.84,width:300,height:245}},
  "mesh shorts":{bottom:{x:0,y:-4,scale:0.84,width:300,height:245}},
  shorts:{bottom:{x:0,y:-4,scale:0.84,width:300,height:245}},
  pants:{bottom:{x:0,y:4,scale:0.82,width:300,height:355}},
  jeans:{bottom:{x:0,y:4,scale:0.82,width:300,height:355}}
};
...
const getFitConfig=(item,section)=>{
  ...
  const productPosition=item?.outfitPosition||{};

  return{
    ...categoryPreset,
    x:productPosition.x!==undefined?Number(productPosition.x):categoryPreset.x,
    y:productPosition.y!==undefined?Number(productPosition.y):categoryPreset.y,
    scale:productPosition.scale!==undefined?Number(productPosition.scale):categoryPreset.scale
  };
};
...
const imageUrlToBase64=async(url)=>{
  ...
  return new Promise((resolve,reject)=>{
    ...
    reader.onloadend=()=>{
      ...
      const mimeType=meta?.match(/data:(.*);base64/)?.[1]||blob.type||"image/png";
...
const StyleBuilder=()=>{
  ...
  const getProductSection=(product)=>{
    ...
    const match=categoryMeta.find((cat)=>{
      ...
      return(
        (catName&&catName===productCategory)||
        (catId&&catId===productCategoryId)||
        (catId&&catId===productCategory)
      );
    });
    ...
    if(
      ...
      searchable.includes("t-shirt")||
      searchable.includes("tee")||
      searchable.includes("shirt")||
      searchable.includes("jersey")||
      searchable.includes("long sleeve")||
      searchable.includes("longsleeve")||
    ...
    if(
      searchable.includes("mesh short")||
      searchable.includes("mesh shorts")||
      searchable.includes("short")||
      searchable.includes("jort")||
      searchable.includes("pants")||
      searchable.includes("pant")||
      searchable.includes("jeans")||
  ...
  const addToFit=(product)=>{
    ...
    if(section!=="top"&&section!=="bottom"){
      console.warn("[STYLE BUILDER] Cannot add unclassified product:",product);
      return;
    }
  ...
  const generateAISuggestion=async()=>{
    try{
      ...
      if(!token){
        setAiError("You are not logged in. Please log in before using AI Style Analysis.");
        return;
      }

      if(!backendUrl){
        setAiError("Backend URL is not configured.");
        return;
      }
    ...
    try{
      ...
      const response=await axios.post(
        `${backendUrl}/api/ai/generate-fit-image`,
        {
          ...
          style:`Generate a realistic full-body Saint Clothing mannequin wearing the selected clothing.

If only a top is provided, generate only the selected top naturally fitted to the mannequin.

If only a bottom is provided, generate only the selected bottom naturally fitted to the mannequin.

If both are provided, generate the complete outfit.
...
No additional accessories.`
  ...
  const renderProductCard=(item,active)=>{
    ...
    return(
      <button
        ...
        } ${mode==="automatic"?"cursor-default opacity-70":""}`}
      >
        <div className={`relative aspect-[1.15] overflow-hidden ${active?"bg-white":"bg-[#f4f1eb]"}`}>
          <img
        ...
        <div className="p-3">
          <p className={`line-clamp-1 text-[10px] font-black uppercase tracking-tight ${active?"text-white":"text-black"}`}>
            {item.name}
          </p>

          <div className="mt-1 flex items-center justify-between">
            <p className={`text-[10px] font-semibold ${active?"text-white/60":"text-black/50"}`}>
              {currency}{getFinalPrice(item).toLocaleString()}
            </p>

            <p className={`text-[8px] font-black uppercase tracking-widest ${active?"text-white/40":"text-black/30"}`}>
              {section}
            </p>
          </div>
        </div>
  ...
  return(
    <div className="min-h-screen bg-[#f5f3ef] px-3 py-4 md:px-5 lg:px-6">
      <style>
        {`
          @keyframes saintFade{from{opacity:0;transform:scale(.985)}to{opacity:1;transform:scale(1)}}
          @keyframes saintPulse{0%,100%{opacity:.55}50%{opacity:1}}
          ...
          .saint-scroll::-webkit-scrollbar{width:5px}
          .saint-scroll::-webkit-scrollbar-track{background:transparent}
          .saint-scroll::-webkit-scrollbar-thumb{background:#c9c9c9;border-radius:999px}
        ...
        <header className="mb-3 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex w-full rounded-[8px] border border-black/10 bg-white p-1 sm:w-[250px]">
            ...
            <button
              ...
              className={`flex-1 rounded-[6px] py-2.5 text-[10px] font-black uppercase tracking-widest transition ${
                mode==="automatic"?"bg-black text-white":"text-black/50 hover:text-black"
              }`}
            >
              AI Assist
            </button>
        ...
        <div className="grid grid-cols-1 gap-3 xl:grid-cols-[235px_minmax(430px,1fr)_300px]">
            ...
            <div className="mt-4 border-t border-black/10 pt-4">
              ...
              <div className="mt-4">
                ...
                <p className="mt-2 text-[8px] leading-relaxed text-black/35">
                  Drag the selected garment directly on the mannequin for fine positioning.
                </p>
              </div>
            </div>
          ...
          <main className="order-1 min-w-0 xl:order-2">
            <section
              ...
              style={{background:generatedImage?"#050505":previewBg}}
            >
              <div className="absolute left-4 top-4 z-[100]">
                ...
                <p className="mt-1 text-[10px] font-semibold text-white/30 mix-blend-difference">
                  {generatedImage?"AI GENERATED":"MANUAL FIT"}
                </p>
              </div>
              ...
              {imageLoading&&(
                <div className="absolute left-1/2 top-1/2 z-[200] -translate-x-1/2 -translate-y-1/2">
                  <div className="flex min-w-[220px] flex-col items-center rounded-[12px] border border-white/10 bg-black/90 px-7 py-5 text-center text-white shadow-2xl backdrop-blur-md">
                    ...
                    <p className="saint-pulse mt-1 text-[9px] font-medium text-white/45">
              ...
              {imageError&&(
                <div className="absolute bottom-14 left-4 right-4 z-[100] rounded-[7px] bg-red-50 px-3 py-2 text-[10px] font-bold text-red-600">
                  {imageError}
                </div>
              )}
                  ...
                  >
                    ...
                    {selectedBottom&&bottomFit&&(
                      <div
                        ...
                        style={{
                          width:bottomFit.width,
                          height:bottomFit.height,
              ...
              <div className="absolute bottom-3 left-1/2 z-[100] flex -translate-x-1/2 items-center gap-1 rounded-[8px] border border-black/10 bg-white/95 p-1 shadow-lg backdrop-blur">
                <button
                  type="button"
                  onClick={()=>setZoom((value)=>Math.max(0.85,value-0.05))}
                ...
                <button
                  type="button"
                  onClick={()=>setZoom((value)=>Math.min(1.15,value+0.05))}
                  className="flex h-8 w-8 items-center justify-center rounded-[6px] text-sm font-black hover:bg-black/5"
                >
                  +
                </button>
            ...
            <section className="mt-3 rounded-[10px] border border-black/10 bg-[#fbfaf7] p-3">
              <div className="flex flex-col gap-2 sm:flex-row">
                <button
                  ...
                  disabled={aiLoading||(!selectedTop&&!selectedBottom)||mode==="automatic"}
                ...
                >
                  <span>✦</span>
                  {aiLoading?"Analyzing...":"AI Style Analysis"}
                </button>
                ...
                >
                  <span>✦</span>
                  {imageLoading?"Generating...":"Generate Outfit"}
                </button>
              </div>
          ...
          <aside className="order-3 min-w-0 rounded-[10px] border border-black/10 bg-[#fbfaf7] p-3">
            {productsLoading&&(
              <div className="mb-3 rounded-[7px] bg-black/5 p-3 text-center text-[9px] font-bold text-black/40">
                Loading products...
              </div>
            )}

            {!productsLoading&&productsError&&(
              <div className="mb-3 rounded-[7px] bg-red-50 p-3 text-[9px] font-bold text-red-600">
                {productsError}
              </div>
            )}
            ...
            <div className="mt-4 border-t border-black/10 pt-4">
              ...
              <div className="mt-3">
                {!productsLoading&&bottomOptions.length===0?(
                  ...
                  paginatedBottomOptions.map((item)=>
            ...
            {mode==="automatic"&&(
              <div className="mt-4 rounded-[8px] bg-black p-3 text-white">
                ...
                <p className="mt-2 text-[9px] leading-relaxed text-white/50">
                  AI automatically selects a matching top and bottom and generates your outfit preview.
                </p>
              </div>
            )}
          </aside>
