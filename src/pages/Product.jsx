...
const Product = () => {
  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-[#F7F7F4] pt-3 pb-8">
        <div className="max-w-7xl mx-auto px-3 sm:px-4 lg:px-6">

          <div className="grid grid-cols-1 lg:grid-cols-[0.95fr_1.05fr] gap-4 lg:gap-5 items-start">

            {/* LEFT IMAGE AREA */}
            <div className="bg-white border border-black/10 rounded-[20px] shadow-sm overflow-hidden lg:sticky lg:top-24">
              <div className="grid grid-cols-1 sm:grid-cols-[76px_1fr]">

                <div className="order-2 sm:order-1 border-t sm:border-t-0 sm:border-r border-black/10 p-2">
                  <div className="flex sm:flex-col gap-2 overflow-x-auto sm:overflow-y-auto sm:max-h-[520px] scrollbar-thin-hide">
                    {Array.isArray(productData.images) && productData.images.length > 0 ? (
                      productData.images.map((img, idx) => {
                        ...
                        return (
                          <button
                            ...
                            className={`shrink-0 w-16 h-16 sm:w-full sm:h-[72px] rounded-[14px] overflow-hidden transition ${isActive
                                ? "ring-2 ring-black"
                      ...
                      <div className="text-xs font-bold uppercase text-gray-400 p-3">
                        No images
                      </div>
                    )}
                  </div>
                ...
                <div className="order-1 sm:order-2">
                  <div className="relative aspect-square bg-white overflow-hidden">
                    {productData.onSale && Number(productData.salePercent) > 0 && (
                      <div className="absolute top-3 right-3 z-20 bg-red-600 text-white text-[10px] font-black uppercase px-3 py-1.5 rounded-full shadow">
            ...
            <div className="space-y-4">
              ...
              <div className="bg-white border border-black/10 rounded-[20px] shadow-sm p-4 sm:p-5">
                <p className="text-[10px] font-black uppercase tracking-[0.25em] text-gray-400">
                  Streetwear Archive
                </p>

                <h1 className="mt-2 text-2xl sm:text-3xl lg:text-4xl font-black italic uppercase leading-none text-[#0A0D17]">
                  {productData.name}
                </h1>

                <div className="mt-3 flex flex-wrap gap-2">
                  <span className="px-3 py-1.5 rounded-full bg-black text-white text-[10px] font-black uppercase tracking-[0.14em]">
                    {productData.category || "Product"}
                  </span>

                  <span className="px-3 py-1.5 rounded-full bg-[#F3F3F0] border border-black/10 text-[10px] font-black uppercase tracking-[0.14em] text-gray-600">
                    Color: {displayColor}
                  </span>

                  {productData.onSale && Number(productData.salePercent) > 0 && (
                    <span className="px-3 py-1.5 rounded-full bg-red-600 text-white text-[10px] font-black uppercase tracking-[0.14em]">
                      Sale
                    </span>
                  )}
                </div>
                ...
                <div className="mt-4 rounded-[18px] border border-black/10 bg-[#FAFAF8] p-4">
                  {isLoggedIn ? (
                    productData.onSale && Number(productData.salePercent) > 0 ? (
                      <>
                        <p className="text-sm font-black text-gray-400 line-through italic">
                          {currency}
                          {Number(productData.price || 0).toFixed(2)}
                        </p>

                        <div className="mt-1 flex items-center gap-2 flex-wrap">
                          <p className="text-3xl sm:text-4xl font-black italic text-red-600 leading-none">
                            {currency}
                            {finalPrice}
                          </p>
                      ...
                      <p className="text-3xl sm:text-4xl font-black italic text-[#0A0D17] leading-none">
                        {currency}
                        {Number(productData.price || 0).toFixed(2)}
                      </p>
                    )
                    ...
                    <div className="flex gap-2 flex-wrap">
                      {colorVariants.map((variant) => (
                        <button
                          ...
                          className={`flex items-center gap-2 px-3 py-2 rounded-xl border transition ${String(variant._id) === String(productData._id)
                    ...
                    {availableSizes.map((s) => {
                      ...
                      return (
                        <button
                          ...
                          disabled={isOut}
                          className={`relative shrink-0 min-w-[52px] px-3 py-2.5 rounded-xl border text-sm font-black uppercase transition ${size === s
                        ...
                        >
                          ...
                          {isPreferred && !isOut && (
                            <span
                              className={`absolute -top-2 -right-2 px-1.5 py-0.5 rounded-full text-[8px] font-black uppercase ${size === s ? "bg-white text-black" : "bg-black text-white"
                                }`}
                            >
                              Pref
                            </span>
                ...
                {productData?.sizeChartImage && (
                  <div className="mt-3 flex items-center justify-between gap-3 rounded-[16px] border border-black/10 bg-[#FAFAF8] px-4 py-3">
                    ...
                    <button
                      ...
                      className="rounded-full bg-black text-white px-4 py-2 text-[10px] font-black uppercase tracking-[0.14em]"
                    >
                      View
                    </button>
                  </div>
                ...
                <div className="mt-4">
                  ...
                  <div className="inline-flex items-center rounded-xl overflow-hidden border border-black/10 bg-[#F6F6F3]">
                    <button
                      ...
                      className="w-10 h-10 text-lg font-black hover:bg-black hover:text-white transition"
                    >
                      −
                    </button>

                    <span className="min-w-[44px] h-10 flex items-center justify-center text-sm font-black bg-white border-x border-black/10">
                      {quantity}
                    </span>
                    ...
                    <button
                      ...
                      onClick={() => {
                        ...
                        if (selectedStock <= 0) return toast.error(`Size ${size} is out of stock`);
              ...
              <div className="bg-white border border-black/10 rounded-[20px] shadow-sm p-4 sm:p-5">
                <div className="flex items-center justify-between gap-3 mb-3">
                  ...
                  {isProductOutOfStock ? (
                    <span className="inline-flex items-center gap-2 rounded-full border border-red-200 bg-red-50 px-3 py-1.5 text-[10px] font-black uppercase tracking-[0.16em] text-red-600">
                      <span className="h-2 w-2 rounded-full bg-red-500 animate-pulse" />
                      Out of Stock
                    </span>
                  ) : (
                    <span className="inline-flex items-center gap-2 rounded-full border border-green-200 bg-green-50 px-3 py-1.5 text-[10px] font-black uppercase tracking-[0.16em] text-green-700">
                ...
                <div className="grid grid-cols-2 gap-2.5">
                  <button
                    ...
                    className={`h-11 rounded-xl font-black uppercase tracking-[0.12em] text-sm transition ${isProductOutOfStock
                  ...
                  <button
                    ...
                    disabled={isProductOutOfStock}
                    className={`h-11 rounded-xl border-2 font-black uppercase tracking-[0.12em] text-sm transition ${isProductOutOfStock
                  ...
                  <button
                    ...
                    className="h-11 rounded-xl bg-[#111] text-white font-black uppercase tracking-[0.12em] text-sm hover:opacity-90 transition"
                  >
                    Try It On
                  </button>
                  ...
                  <button
                    ...
                    className={`h-11 rounded-xl border-2 font-black uppercase tracking-[0.12em] text-sm transition ${has3DModel
                        ? "border-black bg-white text-black hover:bg-black hover:text-white"
                        : "border-gray-300 bg-gray-100 text-gray-400 cursor-not-allowed"
                      }`}
                  >
          ...
          <div
            id="reviews-section"
            className="mt-5 bg-white border border-black/10 rounded-[20px] shadow-sm p-4 sm:p-5"
          >
            <div className="flex gap-2 flex-wrap border-b border-black/10 pb-3 mb-4">
              {[
                ...
                <button
                  ...
                  className={`px-3 py-2 rounded-full text-[11px] sm:text-sm font-black uppercase tracking-[0.08em] transition ${activeTab === key
                      ? "bg-black text-white"
                  ...
                  availableBranches.map((item) => (
                    ...
                    >
                      <div className="flex items-center justify-between gap-3 mb-2">
                        ...
                        <span
                          className={`px-3 py-1 rounded-full text-[10px] font-black uppercase ${item.available
                      ...
                      <p className="text-sm text-gray-600">
                        Branch Code: <span className="font-bold uppercase">{item.branch}</span>
                      </p>

                      <p className="text-sm text-gray-600 mt-1">
                        Stock: <span className="font-bold">{item.totalStock}</span>
                      </p>
                    </div>
                  ))
                ) : (
          ...
          {styleRecommendations.length > 0 && (
            <div className="mt-5 bg-white border border-black/10 rounded-[20px] shadow-sm p-4 sm:p-5">
