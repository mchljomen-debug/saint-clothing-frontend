...
const getStockValue = (stock, size) => {
  ...
  if (typeof stock.get === "function") {
    return (
      Number(
        stock.get(target) ??
          stock.get(target.toLowerCase())
      ) || 0
    );
  }

  if (typeof stock === "object") {
    const entry = Object.entries(stock).find(
...
const getColorLabel = ({ color, colorHex }) =>
  color?.trim() || colorHex?.trim() || "Default";
...
const Product = () => {
  ...
  */
  ...
  const validPid =
    pid &&
    !["undefined", "null"].includes(String(pid));
  ...
  */
  ...
  const colorVariants = useMemo(() => {
    ...
    const sameGroup = products.filter(
      (item) =>
        item &&
        normalizeValue(item.groupCode) ===
          normalizeValue(productData.groupCode) &&
        !item.isDeleted
    );
    ...
    return sameGroup.filter(
      (item, index, array) =>
        ...
        array.findIndex(
          (x) =>
            normalizeValue(x.color) ===
              normalizeValue(item.color) &&
            normalizeValue(x.colorHex) ===
              normalizeValue(item.colorHex)
        )
    );
  }, [
    products,
  ...
  */
  ...
  const availableSizes = useMemo(() => {
    ...
    const stockSizes =
      ...
      typeof productData.stock === "object"
        ? Object.keys(productData.stock).map(
  ...
  const isSizeAvailable = useCallback(
    (targetSize) => {
      ...
      const preorder = Number(
        normalizedPreorderStock[targetSize] || 0
      );
  ...
  useEffect(() => {
    ...
    if (
      preferred &&
      availableSizes.includes(preferred) &&
      isSizeAvailable(preferred)
    ) {
      setSize(preferred);
      return;
    }
    ...
    setSize(
      availableSizes.find(isSizeAvailable) || ""
    );
  }, [
    availableSizes,
    isSizeAvailable,
  ...
  */
  ...
  const selectedActualStock = size
    ? Number(normalizedStock[size] || 0)
    : 0;

  const selectedPreorderStock = size
    ? Number(
        normalizedPreorderStock[size] || 0
      )
    : 0;

  const totalProductStock = getTotalStock(
    productData?.stock
  );

  const totalPreorderStock = getTotalStock(
    productData?.preorderStock
  );
  ...
  */
  ...
  const availableBranches = useMemo(() => {
    ...
    |--------------------------------------------------------------------------
    | FIX:
    | The old code compared item.sku to item.sku.
    | That made EVERY product with a SKU match.
    |--------------------------------------------------------------------------
    */
      ...
      */
      ...
      if (
        ...
        return (
          ...
          normalizeValue(productData.sku)
        );
      }
    ...
    const matchingProducts =
      products.filter(matchesProduct);

    /*
    |--------------------------------------------------------------------------
    | Build fallback branch list
    |--------------------------------------------------------------------------
    */
  ...
  */
  ...
  /*
  |--------------------------------------------------------------------------
  | Clear warning when valid selection is made
  |--------------------------------------------------------------------------
  */
    ...
    (buttonElement) => {
      ...
      Object.assign(
        flyer.style,
        {
          ...
          boxShadow:
            "0 20px 55px rgba(0,0,0,0.30), 0 5px 15px rgba(0,0,0,0.15)",
          willChange:
  ...
  */
  ...
  const validatePurchase = () => {
    ...
    if (quantity > selectedStock) {
      showWarning(
        isSelectedSizePreOrder
          ? `Only ${selectedStock} pre-order slot${
              selectedStock === 1
                ? ""
                : "s"
            } left for size ${size}.`
  ...
  */
  ...
  const handleShow3D = () => {
    if (!isLoggedIn) {
      showWarning(
        "Please login first to use the 3D product view."
      );
    ...
    if (!has3DModel) {
      showWarning(
        "No 3D model or video is attached to this product."
      );
  ...
  return (
    <>
      <style>{`
        ...
        .product-page-enter {
          animation: productFadeIn .55s ease both;
        }
        ...
        .warning-container {
          animation: warningIn .22s ease both;
        }
                  ...
                  <div className="mt-8 grid grid-cols-2 gap-px border border-black/10 bg-black/10 sm:grid-cols-3">
                    {[
                      ...
                      ([label, value]) => (
                        <div
                          key={label}
                          className="bg-[#ECEAE4] p-4 sm:p-5"
                        >
                        ...
                        (item) => (
                          ...
                          >
                            <div className="flex items-start justify-between gap-4">
                              ...
                              <span
                                className={`shrink-0 px-2 py-1 text-[7px] font-black uppercase tracking-[0.12em] ${
                                  item.available
                                    ? "bg-black text-white"
              ...
              <div className="flex-1 overflow-y-auto p-5 sm:p-6">
                ...
                <div className="mt-6 border-t border-black/10 pt-5">
                  ...
                  <p className="mt-3 text-sm leading-7 text-black/55">
                    Compare your body measurements
                    with the chart for a better fit.
                    If you want a looser streetwear
                    look, choose one size up from your
                    regular fit.
                  </p>
                </div>
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-[1.1fr_0.9fr]">
              <div className="border-b border-white/10 p-6 md:border-b-0 md:border-r">
                ...
                <p className="mt-5 max-w-md text-sm leading-6 text-white/45">
                  Scan the QR code or open the app
                  on your phone to continue using
                  Saint Clothing Try It On.
                </p>
              ...
              <aside className="overflow-y-auto border-t border-black/10 bg-[#F5F4F0] p-5 sm:p-6 lg:border-l lg:border-t-0">
                ...
                <div className="mt-8 grid gap-2">
                  <button
                    ...
                    className="flex h-12 items-center justify-between bg-black px-4 text-[9px] font-black uppercase tracking-[0.18em] text-white transition hover:bg-black/85"
                  >
                    Add to Cart
