  ...
  return(
    <div className="min-w-0 lg:pt-2">
      <div className="flex items-center justify-between gap-4">
        <p className="text-[9px] font-black uppercase tracking-[0.3em] text-black/40 sm:text-[10px]">
          {productData.category||"Product"}
        </p>

        <div className="flex flex-wrap items-center justify-end gap-2">
          {isProductPreOrder&&!isProductOutOfStock&&(
            <span className="inline-flex items-center gap-2 bg-black px-3 py-1.5 text-[8px] font-black uppercase tracking-[0.18em] text-white">
          ...
          {productData.bestseller&&(
            <span className="border border-black px-2.5 py-1.5 text-[8px] font-black uppercase tracking-[0.18em]">
              Bestseller
            </span>
          )}
        </div>
      ...
      {isProductPreOrder&&!isProductOutOfStock&&(
        <div className="mt-6 border-2 border-black bg-black p-5 text-white">
          <div className="flex items-start justify-between gap-5">
            ...
            <span className="shrink-0 border border-white/25 px-3 py-2 text-[8px] font-black uppercase tracking-[0.18em]">
              Pre-Order
            </span>
          </div>

          <p className="mt-4 max-w-lg text-[11px] font-medium leading-6 text-white/65">
            Some sizes of this product are currently available through pre-order. Check the size options below before adding the item to your cart.
          </p>
          ...
          <div className="mt-5 border-t border-white/15 pt-4">
            ...
            <p className="mt-1.5 text-[11px] font-black uppercase tracking-[0.1em]">
              {expectedRestockDate}
            </p>
          </div>
          ...
          {productData.preorderNote&&(
            <div className="mt-4 border-t border-white/15 pt-4">
              ...
              <p className="mt-1.5 text-[11px] font-medium leading-5 text-white/65">
                {productData.preorderNote}
              </p>
            </div>
          )}
      ...
      <div className="mt-6 flex flex-wrap gap-2">
        ...
        {!isProductOutOfStock&&isProductSellingFast&&(
          <span className="inline-flex items-center border border-black px-3 py-2 text-[9px] font-black uppercase tracking-[0.17em]">
            Selling Fast
          </span>
        )}
      </div>
      ...
      <div className="flex items-center gap-4">
        <div className="flex items-center gap-0.5">
          {[1,2,3,4,5].map((star)=>(
            <span
              key={star}
              className={`text-[13px] ${star<=Math.round(Number(averageRating))?"text-black":"text-black/15"}`}
            >
              ★
            </span>
          ))}
        ...
        <button
          ...
          className="text-[10px] font-bold uppercase tracking-[0.1em] underline underline-offset-4 transition hover:text-black"
        >
        ...
        <div className="mt-8">
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            {colorVariants.map((variant)=>{
              const active=String(variant._id)===String(productData._id);
              const image=variant.images?.[0]?getMediaUrl(variant.images[0],backendUrl):"";
              ...
              return(
                <button
                  ...
                  onClick={()=>{
                    if(!variant._id||active)return;
                    navigate(`/product/${variant._id}`);
                    window.scrollTo({top:0,behavior:"smooth"});
                  }}
                  className={`group relative h-12 w-12 overflow-hidden border transition ${active?"border-black":"border-black/10 hover:border-black"}`}
                >
                  <span
      ...
      <div className="mt-9">
        ...
        <div className="mt-4 grid grid-cols-3 gap-x-2 gap-y-6 sm:grid-cols-6">
          {availableSizes.map((s)=>{
            ...
            const pre=preorderEnabled&&actual<=0&&preorder>0;
            const out=actual<=0&&(!preorderEnabled||preorder<=0);
            const preferred=String(user?.preferences?.preferredSize||"").toUpperCase()===s;
            ...
            return(
              <button
                ...
                className={`relative h-14 border text-[10px] font-black uppercase tracking-[0.14em] transition ${
                  size===s
                    ?"border-black bg-black text-white"
                    :"border-black/10 bg-transparent text-black hover:border-black"
                } ${out?"cursor-not-allowed opacity-25 line-through":""}`}
              >
                {s}

                {preferred&&!out&&!pre&&(
                  <span className={`absolute -right-px -top-px px-1.5 py-1 text-[6px] font-black uppercase tracking-[0.08em] ${
                    size===s?"bg-white text-black":"bg-black text-white"
                  }`}>
                    Pref
                  </span>
                )}
                ...
                {pre&&!out&&(
                  <span className={`absolute bottom-0 left-0 right-0 py-1 text-center text-[6px] font-black uppercase tracking-[0.1em] ${
                    size===s?"bg-white text-black":"bg-black text-white"
                  }`}>
                    Pre-Order
                  </span>
        ...
        {isSelectedSizePreOrder&&(
          <div className="mt-6 border-2 border-black bg-[#ECEAE4]">
            <div className="flex items-center justify-between gap-4 bg-black px-4 py-3 text-white">
              ...
              <p className="text-[8px] font-black uppercase tracking-[0.12em]">
                {selectedPreorderStock} Slot{selectedPreorderStock!==1?"s":""} Left
              </p>
            </div>

            <div className="p-4">
              <p className="text-[11px] font-bold leading-5">
                Size {size} is a pre-order item.
              </p>
              ...
              {productData.preorderNote&&(
                <p className="mt-3 border-t border-black/10 pt-3 text-[10px] font-medium leading-5 text-black/55">
                  {productData.preorderNote}
                </p>
              )}
            </div>
      ...
      <div className="mt-8">
        {isSelectedSizePreOrder&&(
          <div className="mb-2 flex items-center justify-center gap-2 border border-black bg-[#ECEAE4] px-4 py-2.5">
            <span className="h-1.5 w-1.5 rounded-full bg-black"/>
            <p className="text-[8px] font-black uppercase tracking-[0.18em]">
              You are adding a pre-order item
            </p>
          </div>
        )}
        ...
        <button
          ...
          disabled={isProductOutOfStock||addingToCart}
          className={`group flex h-14 w-full items-center justify-between px-5 text-[10px] font-black uppercase tracking-[0.2em] transition sm:px-6 ${
            isProductOutOfStock||addingToCart
        ...
        <button
          ...
          className={`group mt-2 flex h-14 w-full items-center justify-between border px-5 text-[10px] font-black uppercase tracking-[0.2em] transition sm:px-6 ${
            isProductOutOfStock
