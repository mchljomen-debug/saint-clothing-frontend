...
const ForYouCollection = () => {
  ...
  useEffect(() => {
    ...
    const personalized = [...products]
      .filter((item) => favoriteCategories.includes(item.category))
      .sort((a, b) => {
    ...
    setForYouProducts(personalized);
  }, [products, favoriteCategories]);

  if (favoriteCategories.length === 0 || forYouProducts.length === 0) {
    return null;
  }

  const isSliding = forYouProducts.length > 5;
      ...
      <div className="px-0 md:px-2">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 md:gap-8 mb-8 md:mb-10">
          <div className="text-left max-w-2xl">
            ...
            <p className="max-w-xl mt-4 text-[13px] md:text-sm text-gray-500 leading-relaxed">
              Pieces selected around your favorite categories for a more personal
              Saint Clothing shopping experience.
            </p>
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              {favoriteCategories.map((cat) => (
                <span
                  key={cat}
                  className="rounded-full border border-black/10 bg-black text-white px-3 py-1 text-[10px] font-black uppercase tracking-[0.14em]"
                >
                  {cat}
                </span>
              ))}
        ...
        >
          {forYouProducts.map((item) => (
            ...
            >
              <ProductItem {...item} badgeMode="none" forYou={true} />
            </div>
          ))}
        </div>
